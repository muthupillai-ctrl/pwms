...
export const MfController = {
  ...
  async updateFund(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const fund = await mfService.updateFund(req.params['id'], req.user!.sub, req.body);
      ok(res, { fund });
    } catch (err) { next(err); }
  },
  ...
  async deleteFund(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      await mfService.deleteFund(req.params['id'], req.user!.sub);
      noContent(res);
    } catch (err) { next(err); }
  },
  ...
  async addTransaction(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const txn = await mfService.addTransaction(req.params['fundId'], req.user!.sub, req.body);
  ...
  async deleteTransaction(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      await mfService.deleteTransaction(req.params['txnId'], req.user!.sub);
      noContent(res);
    } catch (err) { next(err); }
  },
  ...
  async getNav(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const result = await fetchNav(req.params['schemeCode']);
      ok(res, { nav: result });
    } catch (err) { next(err); }
  },
};
